...
import { buildPointRateMap, computeEarnedPoint } from './helper';
import { getCatalogCacheValue, perbaharuiMembership } from '../../utils/cache';
...
const buildRateMap = () => buildPointRateMap(getCatalogCacheValue('categories') || []);
...
export function mirrorMembershipOrder(order) {
  ...
  return updateCache(customer, clone => {
    ...
    const earned = computeEarnedPoint(order?.items, buildRateMap());
    if (earned > 0) {
 ...
 * Tanpa ini, point/saldo lokal drift (inflated) sampai refetch online.
 */
